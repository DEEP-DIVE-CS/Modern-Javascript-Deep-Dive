...
- 희소배열의 경우 존재하지 않는 요소는 순회대상에서 제외 (map. filter, reduce 메서드 등에서도 마찬가지)
  ...
  ```js
  // 희소 배열const arr = [1, , 3];// for 문으로 희소 배열을 순회for (let i = 0; i < arr.length; i++) {  console.log(arr[i]); // 1, undefined, 3 }// forEach 메서드는 희소 배열의 존재하지 않는 요소를 순회 대상에서 제외한다. arr.forEach(v => console.log(v)); // 1, 3
  ```
